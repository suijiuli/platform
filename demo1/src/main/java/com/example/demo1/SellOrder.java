...
public class SellOrder extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try (PrintWriter out = resp.getWriter();
             Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/mysql?useUnicode=true&characterEncoding=UTF-8&serverTimezone=Asia/Shanghai&useSSL=false", "root", "123456");
             PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM orders WHERE merchant=?")) {
            ...
            out.println("<style>");
            out.println("table {");
            out.println("  width: 100%; border-collapse: collapse;"); // 合并边框
            out.println("}");
            out.println("th, td {");
